...
//	public static String myEmailSMTPHost = "smtp.163.com";
	...
	// PS: 某些邮箱服务器为了增加邮箱本身密码的安全性，给 SMTP 客户端设置了独立密码（有的邮箱称为“授权码”）,
	// 对于开启了独立密码的邮箱, 这里的邮箱密码必需使用这个独立密码（授权码）。
	public static String myEmailAccount = "service@ioesy.cn";
	public static String myEmailPassword = "ioesy@2017";
	
	// 发件人邮箱的 SMTP 服务器地址, 必须准确, 不同邮件服务器地址不同, 一般(只是一般, 绝非绝对)格式为: smtp.xxx.com
	// 网易163邮箱的 SMTP 服务器地址为: smtp.163.com
	public static String myEmailSMTPHost = "smtp.chengmail.cn";
...
//	public static String receiveMailAccount = "269247831@qq.com";
	...
	public static MimeMessage createMimeMessage(Session session, String sendMail, String receiveMail, String name, String verify) throws Exception {
		...
		// 2. From: 发件人
		message.setFrom(new InternetAddress(sendMail, "cms系统", "UTF-8"));

		// 3. To: 收件人（可以增加多个收件人、抄送、密送）
		message.setRecipient(RecipientType.TO, new InternetAddress(receiveMail, name, "UTF-8"));

		// 4. Subject: 邮件主题
		message.setSubject("cms系统", "UTF-8");
		String content = "<style type='text/css'>.p{text-indent: 2em;}</style>" + name + "，您好：<br /><div class='p'>您于" + getStringDate() + "在cms系统上进行了找回密码操作</div><br />（如非本人操作，则密码可能已经泄露，建议联系管理员修改）<br /><br /><div class='p'>验证码为："+verify+" </div><br />（在三分钟内有效，3分钟后需要重新提交）";
