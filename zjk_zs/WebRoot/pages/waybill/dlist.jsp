  ...
  <body>
    ...
    <form action="" method="post" id="form_do" name="form_do">
        <table width="100%">
			...
			<%-- <c:if test="${c.qrcode.hasparent==0 }"> --%>
				<tr>
	                <td>${s.index+1 }</td>
	                ...
	                <td>${c.deliverenterprise.company }</td>
	                <td>${c.deliveremployee.name }</td>
	                <td><date:date value="${c.delivertime }"></date:date></td>
	                <td>
					...
					<td>
					<c:if test="${!empty c.qrcode.son.product.tradename }">
						<a href="waybill-dslist.php?waybillid=${waybillid }&qrcodeid=${c.qrcode.id }">查看</a>
	                </c:if>
