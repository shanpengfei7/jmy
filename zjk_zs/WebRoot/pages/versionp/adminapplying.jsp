...
<script type="text/javascript">
	function check_btn(id) {
		if(confirm("您确定要审核通过这个产品吗？")) {
			window.location.href="versionp-check.php?id=" + id + "&ad=1";
		}
	}
	function delete_btn(id) {
		if(confirm("您确定要删除这个产品吗？")) {
			window.location.href="versionp-delete.php?id=" + id + "&ad=1";
		}
	}
</script>
  </head>
  ...
  <body>
    ...
    <div class="operate">
        <div class="left">
                <!-- <input type="button" onclick="btn_apply()" class="btn_blue" value="型号申请" disabled="disabled"> -->
        </div>
        <div class="left_pad">
        ...
        <table width="100%">
			...
			<c:forEach items="${products }" var="p" varStatus="s">
				<tr>
	                ...
	                <td>
	                <c:if test="${p.isapply==0 }">
		                <a href="javascript:delete_btn('${p.id }');">删除</a>
		                <a href="versionp-show.php?id=${p.id }&ad=1">编辑</a>
		                <a href="javascript:check_btn('${p.id }');">审核</a>
		            </c:if>
					</td>
            </tr>
			</c:forEach>
